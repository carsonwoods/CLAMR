...
//
// TODO: I need to ask what is going on with this block of code
...
parse_arguments(int argc, char ** argv) {
  ...
  while (1) {
    ...
    case 'm':
      ...
      break;
    case '?':
      // runs if argument is not specified character
      usage(argv[0]);
      break;
    }
  }
...
void report_results_update(int penum, double * time_total_pe, int count_updated_pe, int num_timings, int type_size) {
  ...
  if (penum == 0) {
    // allocated pointer which will store all bandwidth values
    double *bandwidth_global = (double*) malloc(num_timings * sizeof(double));

    // will store median latency and bandwidth values respectively
    double latency_med, bandwidth_med;

    // will store mean latency and bandwidth values respectively
...
int main(int argc, char * argv[]) {
  ...
  if (ierr == EZCL_NODEVICE) {
    // attempts to initialize OpenCL on CPU if no GPU found
  ...
  #ifdef HAVE_OPENCL
  /* L7
   * uses a different operation for the OpenCL setup.
